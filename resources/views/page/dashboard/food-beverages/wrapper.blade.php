...
@section('content')
    <div class="d-lg-flex justify-content-between align-items-center mb-4">
        ...
        <form class="d-flex gap-2" action="" method="get">
            ...
            <button type="button" class="btn btn_theme ajax_modal_btn" title="Add" data-modal-title="Add Food & Beverage"
                data-modal-size="lg" data-render-route="{{ route('dashboard.food-beverages.create') }}">
    ...
    <div class="card bg_transparent_1">
        <div class="card-body">
            @if (config('app.locale') === 'en')
                @include('page.dashboard.food-beverages.table.en')
            @elseif (config('app.locale') === 'id')
                @include('page.dashboard.food-beverages.table.id')
            @endif
        </div>
        <div class="d-flex justify-content-end">
            {{ $food_beverages->links() }}
        </div>
    </div>
    </div>
@endsection
