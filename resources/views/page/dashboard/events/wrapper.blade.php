...
@section('content')
    <div class="d-lg-flex justify-content-between align-items-center mb-4">
        ...
        <form class="d-flex gap-2" action="" method="get">
            ...
            <button type="button" class="btn btn_theme ajax_modal_btn" title="Add" data-modal-title="Add Event"
                data-modal-size="lg" data-render-route="{{ route('dashboard.events.create') }}">
    ...
    <div class="card bg_transparent_1">
        <div class="card-body">
            @if (config('app.locale') === 'en')
                @include('page.dashboard.events.table.en')
            @elseif (config('app.locale') === 'id')
                @include('page.dashboard.events.table.id')
            @endif
        </div>
