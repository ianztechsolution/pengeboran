        ...
        <div class="card-body">
            <div class="row">
                <div class="col-xxl-12">
                    <form class="tablelist-form" action="{{ route('dashboard.transaction.invoices.store') }}" method="post" class="ajax_form" data-after-submit="reload"
                enctype="multipart/form-data">
                    @csrf
                        <div class="mb-3">
                            ...
                            <select class="form-control bg-transparent" name="project_id" id="project_id" required>
                                ...
                                @foreach($projects as $project)
                                    <option value="{{ $project->id }}" {{ request()->project_id && $project->id == request()->project_id ? 'selected' : '' }}>{{ $project->title }}</option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control" id="customer_name" name="customer_name" placeholder="Masukkan Nama Pelanggan" readonly required="">
                        </div>

                        <div class="mb-3">
                            <label for="invoice_no" class="form-label">Nomor Tagihan <span class="text-danger">*</span></label>
                            <input type="text" class="form-control" id="invoice_no" name="invoice_no" placeholder="Masukkan Nomor Tagihan" readonly required="" value="{{ 'INV-'.time() }}">
                        </div>

                        <div class="mb-3">
                            <label for="total_price" class="form-label">Total Harga <span class="text-danger">*</span></label>
                            <input type="number" class="form-control" id="total_price" name="total_price" placeholder="Masukkan Total Harga" readonly required="" value="0">
                        </div>

                        <div class="mb-3">
                            <label for="dicount" class="form-label">Discount</label>
                            <input type="number" class="form-control" id="dicount" name="dicount" placeholder="Masukkan Diskon" value="0">
                        </div>

                        <div class="mb-3">
                            <label for="tax" class="form-label">Pajak</label>
                            <input type="number" class="form-control" id="tax" name="tax" placeholder="Masukkan Pajak" value="0">
                        </div>

                        <div class="mb-3">
                            <label for="grand_total" class="form-label">Total Tagihan <span class="text-danger">*</span></label>
                            <input type="number" class="form-control" id="grand_total" name="grand_total" placeholder="Masukkan Total Tagihan" readonly required="" value="0">
                        </div>

                        <div class="mb-3">
                            <label for="total_payment" class="form-label">Total Dibayar</label>
                            <input type="number" class="form-control" id="total_payment" name="total_payment" placeholder="Masukkan Total Pembayaran" value="0">
                        </div>

                        <div class="mb-3">
                            <label for="paid_date" class="form-label">Tanggal Dibayar</label>
                            <input type="date" class="form-control" id="paid_date" name="paid_date" placeholder="Masukkan Tanggal Dibayar">
                        </div>
                        ...
                        <div class="hstack gap-2 justify-content-end mb-3">
                            ...
                            <button type="submit" class="btn btn-success" id="add-btn">Buat Tagihan</button>
                        </div>
                    </form>
                </div><!--end col-->
            </div><!--end row-->
