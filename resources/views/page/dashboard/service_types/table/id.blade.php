    ...
    <table id="service-types-datatables" class="display table dt-responsive table-borderless table-centered align-middle table-nowrap mb-0">
        <thead class="bg-light bg-opacity-50">
            <tr>
                ...
                <th scope="col" class="sort cursor-pointer text-muted" data-sort="diseases">Harga</th>
                <th scope="col" class="sort cursor-pointer text-muted" data-sort="age">Status</th>
        ...
        <tbody class="list">
            @foreach ($service_types as $service_type)
                <tr>
                    ...
                    <td class="price">
                        {{ \App\Helper\Helper::currencyFormatting($service_type->price, 'Rp. ') }}
                    </td>
                    <td class="status">
                    ...
                    <td class="d-flex align-items-center gap-3 justify-content-end">
                        <a href="{{ route('dashboard.master-data.service-types.show', $service_type->id) }}" class="btn btn-sm btn-info">
                            <i class="bx bx-show"></i>
                        </a>
                        <a href="{{ route('dashboard.master-data.service-types.edit', $service_type->id) }}" class="btn btn-sm btn-warning">
                            <i class="ri-edit-2-line"></i>
                        </a>
                        <form action="{{ route('dashboard.master-data.service-types.destroy', $service_type->id) }}" method="POST" style="display:inline;" data-after-submit="reload">
                            @csrf
                            @method('DELETE')
